...
interface PlayerControlsProps {
  ...
  skipMusic: (trackedId: number) => void;
  queue: Track[];
}

export function PlayerControls({scrollToMusic, skipMusic, queue}: PlayerControlsProps) {
  ...
  return (
    <>
      <Box width={360} height={40} justifyContent="center" alignItems="center" mt="s10">
        <Slider
        ...
        <Box width={330} flexDirection="row" justifyContent="space-between">
          <Text style={styles.progressTxt}>
            {new Date(progress.position * 1000).toISOString().substr(14, 5)}
          </Text>
          <Text style={styles.progressTxt}>{leftTime}</Text>
        </Box>
      </Box>
      ...
      <Box
        ...
        <TouchableOpacity onPress={PreviousMusic}>
          <Ionicons
            ...
            size={35}
            color="#2B7CB5"
            style={styles.buttonControllsChangeMusic}
          />
        </TouchableOpacity>
